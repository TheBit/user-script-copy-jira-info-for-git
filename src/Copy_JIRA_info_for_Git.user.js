...
(function() {
    ...
    function createBranchButton(name, grabbedInfo) {
        let button = document.createElement('button');
        button.classList.add(`${name}Btn`, 'btn', 'btn-sm');
        button.setAttribute('data-clipboard-text', `${name}/${grabbedInfo}`);
    ...
    function handleCommitClick(button, grabbedInfoForCommit) {
        ...
        let result = Array.prototype.reduce.call(commitButtons,
            (sum, currentItem) => currentItem.classList.contains('btn-primary') ?
            `${sum}[${currentItem.dataset.name}]` : sum
            , '');
    ...
    grabbedInfoForBranch = `${$('#key-val').innerText}-${grabbedInfoForBranch}`;
